...
case class STContains(exprs: Seq[Expression]) extends STPredicate {
  ...
  override def left: Expression = exprs.head
  override def right: Expression = exprs.last
  override def predicate: JoinPredicate = JoinPredicate.CONTAINS
...
case class STIntersects(exprs: Seq[Expression]) extends STPredicate {
  ...
  override def left: Expression = exprs.head
  override def right: Expression = exprs.last
  override def predicate: JoinPredicate = JoinPredicate.INTERSECTS
...
case class STContainedBy(exprs: Seq[Expression]) extends STPredicate {
  ...
  override def left: Expression = exprs.head
  override def right: Expression = exprs.last
  override def predicate: JoinPredicate = JoinPredicate.CONTAINEDBY
