...
class DBScan[K, T: ClassTag](var eps: Double = 0.1, var minPts: Int = 10) extends java.io.Serializable {
  ...
  private def performClustering(input: RDD[ClusterPoint[K, T]], globalMBB: MBB): (DBScanModel[K, T], Map[String, Long]) = {
    ...
    logger.info(s"step 0: determining global MBB: $globalMBB")
  ...
  protected[dbscan] def partitionInput(input: RDD[ClusterPoint[K, T]], partitions: List[MBB]): RDD[(Int, ClusterPoint[K, T])] =
    input.flatMap { point =>
      ...
      if (partitionList.isEmpty) {
        println(s"[partitionInput] No partition found for point: ${point.vec.toArray.mkString(",")}")
      }
      //require(partitionList.nonEmpty, s"no partition found for point: ${point.vec.toArray.mkString(",")}")
