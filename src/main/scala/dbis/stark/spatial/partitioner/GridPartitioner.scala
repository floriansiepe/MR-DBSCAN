...
//  def getAllPartitions(key: Any): List[Int]
  ...
  override final def getPartition(key: Any): Int = {
...
//    (minX, maxX + EPS, minY, maxY + EPS)
  ...
  def buildHistogram[G <: STObject, V](rdd: RDD[Envelope], pointsOnly: Boolean, numXCells: Int, numYCells: Int,
    ...
    }.reduceByKey { (a: (NRectRange, Int), b: (NRectRange, Int)) =>
      val (lr, lc) = a
      val (rr, rc) = b
      (lr.extend(rr), lc + rc)
    }.map { case (id: Int, (n: NRectRange, c: Int)) =>
      val bounds: NRectRange = getCellBounds(id, numXCells, xLength, yLength, minX, minY)
