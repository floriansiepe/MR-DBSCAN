...
case class KNN[PayloadT](k: Int) extends Serializable with Cloneable with Iterable[(Distance, PayloadT)] {
  ...
  def min: (Distance, PayloadT) = nn(posMin)
  def max: (Distance, PayloadT) = nn(posMax)
  def full: Boolean = m >= k
  override def isEmpty: Boolean = m < 0
  override def size: Int = if(full) k else m + 1
  def sortedArray: Array[(Distance, PayloadT)] = nn.sortBy(_._1)

  protected[spatial] def set(nns: IndexedSeq[(Distance, PayloadT)]): Unit = {
...
//    println(s"after set: $this")
  ...
  def insert(tuple: (Distance,PayloadT)): Unit = {
    val pos = m+1
    if(pos < k) {
      nn(pos) = tuple
  ...
  def merge(other: KNN[PayloadT]): KNN[PayloadT] = {
    if(isEmpty)
      other.clone()
    else if(other.isEmpty || (full && other.min._1 > max._1))
      this.clone()
    else {
      val knn = this.clone()
