...
class RTree[D: ClassTag ](
  ...
  def insert(mbr: GeoType, data: D): Unit =
    super.insert(mbr.getEnvelopeInternal, new Data(data, mbr))

  def insert(mbr: MBR, data: D): Unit =
...
//  }
  ...
  override def withinDistance(qry: STObject, distFunc: (STObject,STObject) => Distance, maxDist: Distance): Iterator[D] = {
    val env = qry.getGeo.getEnvelopeInternal
    val env2 = new Envelope(
    ...
    super.query(env2).iterator().asScala.map(_.asInstanceOf[Data[D]]).filter { p => distFunc(qry, p.so) <= maxDist }.map(_.data)
  }
   ...
   */
  def _items: Iterator[Data[D]] = super.itemsTree()
      .iterator()
      .asScala
  ...
  override def items: Iterator[D] = _items.map(_.data)
